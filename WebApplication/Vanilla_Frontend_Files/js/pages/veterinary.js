...
const vetState = { specialty: 'all', region: 'all', availability: 'all', species: 'all', search: '' };
...
function applyFilters() {
    ...
    document.querySelectorAll('.vet-dir-card').forEach(card => {
        const { specialty, region, availability, breed } = card.dataset;
        const text = card.textContent.toLowerCase();

        let show = (vetState.specialty === 'all' || specialty.includes(vetState.specialty)) &&
            (vetState.region === 'all' || region === vetState.region) &&
